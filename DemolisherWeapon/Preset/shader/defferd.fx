...
static const float SF = 50000.0f;
static const float SN = 30.0f;
...
inline float ShadowMapFunc(uint usemapnum, float4 worldpos) {
	...
	for (uint i = 0; i < 25; i++) {
		...
		if (blocker_z < lLViewPosition.z) {
			blocker_z = LinearizeDepth(blocker_z, SN, SF);
	...
	//半影のサイズ計算
	float maxCnt = 4.5f*(((LinearizeDepth(lLViewPosition.z, SN, SF) - avg_blocker_z)*(SF - SN)) / (avg_blocker_z * (SF - SN) *((5000.0f - 30.0f)/(SF - SN))));// saturate();
	if (maxCnt <= 0.0f) {
		return 0.0f;
	}
